...
state-transition substrate for decentralized cognitive systems.
...
discretion with a notarized transition algebra. EMP defines:
...
| Ref | Mutable pointer categorized by `trust_level` (e.g., `refs/heads/main`) |
| Desk | Distinguished Notary Principal backed by SQL-RBAC quorum |
| Seal | Cryptographic commit that hardens a belief to finality |
| Sealing Commit | Specialized commit authored by Desk moving a commit from `DISPUTE_OPEN` → `FACT_RETIRED` / `FACT_AFFIRMED` |
